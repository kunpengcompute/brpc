...
int ShmQueue::Open(void* mem, size_t size) {
    ...
    _header = reinterpret_cast<ShmQueueHeader*>(mem);
    if (_header->magic != SHM_QUEUE_MAGIC) {
        LOG(ERROR) << "ShmQueue: invalid magic=" << _header->magic
                   << ", expected=" << SHM_QUEUE_MAGIC;
        return -1;
    }
    ...
    uint32_t esz = _header->elem_size;
    _data = reinterpret_cast<char*>(mem) + hsz;
    _capacity = (size - hsz) / esz;
...
int ShmQueue::WaitNotify(const timespec* abstime) {
    ...
    read(_notify_fd, &val, sizeof(val));
...
PeerShmRegistry::~PeerShmRegistry() {
    ...
    for (auto& pair : _registry) {
        if (pair.second.notify_fd >= 0) {
            close(pair.second.notify_fd);
        }
