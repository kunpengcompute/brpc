...
int ShmBlockAllocator::OpenFromFd(int fd, size_t size) {
    ...
    _header = reinterpret_cast<ShmBufferHeader*>(_base);
    if (_header->magic != SHM_MAGIC) {
        LOG(ERROR) << "ShmBlockAllocator: invalid magic=" << _header->magic
                   << ", expected=" << SHM_MAGIC;
        munmap(_base, size);
        _base = nullptr;
        return -1;
...
butil::IOBuf::Block* ShmBlockAllocator::TryAllocBlock() {
    ...
    while (true) {
        ...
        char* block_data = _block_region + old_head * FLAGS_shm_block_size;
        uint32_t new_head;
        ...
        if (_header->free_head.compare_exchange_weak(old_head, new_head,
            ...
            uint32_t data_cap = FLAGS_shm_block_size - sizeof(butil::IOBuf::Block);
