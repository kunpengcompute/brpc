...
#define DELEGATE(f)                                         \
    ({                                                      \
        FLAGS_ubsocket_enable_wrapper ? UB_API_WRAP(f) : f; \
    })

DEFINE_bool(ubsocket_enable_wrapper, true, "Enable the ubsocket interposition layer. If true, socket calls are routed through ubsocket; otherwise, the original socket api is used.");
