...
inline void FillValue(test::Value* value, int index, const DatasetSpec& spec) {
    switch (index % 8) {
        case 0:
...
inline void FillValueMap(test::ValueMap* value_map, const char* prefix,
...
inline void FillRequest(test::Request* request, const DatasetSpec& spec) {
...
inline uint64_t ConsumeValue(const test::Value& value) {
    uint64_t total = 0;
    switch (value.value_case()) {
        case test::Value::kSValue:
            total += value.s_value().size();
            break;
        case test::Value::kDValue:
            total += static_cast<uint64_t>(value.d_value());
            break;
        case test::Value::kIValue:
            total += static_cast<uint64_t>(value.i_value());
            break;
        case test::Value::kBValue:
            total += value.b_value().size();
            break;
        case test::Value::kSList:
            for (const auto& item : value.s_list().value()) {
                total += item.size();
            }
            break;
        case test::Value::kDList:
            for (double item : value.d_list().value()) {
                total += static_cast<uint64_t>(item);
            }
            break;
        case test::Value::kIList:
            for (int64_t item : value.i_list().value()) {
                total += static_cast<uint64_t>(item);
            }
            break;
        case test::Value::kBList:
            for (const auto& item : value.b_list().value()) {
                total += item.size();
            }
            break;
        case test::Value::VALUE_NOT_SET:
            break;
    }
    return total;
}

inline uint64_t ConsumeValueMap(const test::ValueMap& value_map) {
...
inline uint64_t ConsumeRequest(const test::Request& request) {
